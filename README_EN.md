...
- **v0.3.2** (2026-09-10): micro-interaction layer — hard-edged confetti bursts from the total score when the report card appears; failed answer cards shake on entry; test-connection/run buttons get spinner loading states and are debounced; copying the score text flips the button to "Copied ✓" with a tooltip bubble; SHA-256 level fingerprints are click-to-copy; level checkboxes pop on toggle; `<details>` content fades in; the logo shield wiggles on hover; hardening — content stays visible without IntersectionObserver (fail-open), noscript fallback, and count-up jumps straight to the final value under reduced motion.
- **v0.3.1** (2026-09-10): liveliness layer — hero title pops in character by character, decorations drift with the cursor, scroll progress bar under the nav, endlessly scrolling caution tape; podium crown swing + champion pulse ring; staggered table-row entry; radar polygons grow from center with dot-pop vertices; rank seals wiggle after stamping; button click bursts (ink dots & yellow stars) + icon jiggle on hover; level questions highlight on hover; all motion respects `prefers-reduced-motion`.
