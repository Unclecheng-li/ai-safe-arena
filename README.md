...
> 题库版本 v0.2.1 · 共 **6 关 110 题** · 判分规则与 SHA-256 指纹在[题库页](https://unclecheng-li.github.io/ai-safe-arena/levels.html)实时展示

| 关卡 | 题数 | 考什么 | 判分 |
|------|:---:|--------|------|
| **A1 安全常识** | 20 | 反诈、密码、Wi-Fi、社工、个人信息保护……普通人安全意识 | 选择题精确匹配 |
| **D1 越狱拒答与钓鱼识别** | 25 | 10 题有害/越狱式请求（DAN 包装、"正当理由"话术，该不该拒绝）+ 5 题良性对照（密码生成器/防御演示/自有代码审查等 should-comply 探针，过度拒答即扣分）+ 10 题识骗（AI 换脸、杀猪盘、注销校园贷等 2026 高发） | 拒绝关键词 / 良性对照开头拒答即扣分 / 结论 + 理由关键词组 |
...
| **D3 日志研判与应急响应** | 20 | SSH 爆破、Windows 4625 事件链、crontab 持久化、挖矿进程、Webshell 落地、钓鱼邮件头、DNS 隧道、用户名枚举、Log4Shell、混淆 PowerShell、Redis 未授权、C2 心跳、SSRF、勒索落地、PsExec 横移 + 2 题良性对照（正常备份 / K8s 探活，防"见规律就报警"） | 事件结论 + 归纳性依据关键词（v0.2.1 剔除日志原文词，复读日志不加分） / 字段提取 |
...
| 模块 | 文件 | 说明 |
|------|------|------|
| 判分引擎 | `assets/scoring.mjs` | 八种判分模式，浏览器与 runner 共用 |
...
| 题库 | `benchmark/levels/*.json` | 6 关 110 题，含判分规则与出题解析 |
...
- **v0.4.1**（2026-09-19，题库 v0.2.1）：公平性加固三件套——① D1 新增 5 题良性对照（D1-21~25，新增 `comply` 判分模式，参照 XSTest/OR-Bench 的 should-comply 探针：密码生成器/防御演示/自有代码审查/反诈科普/授权资产梳理，开头拒答即过度拒答扣分），把"过度拒答"测出来；② D3 扩至 20 题（新增用户名枚举提取、Log4Shell、混淆 PowerShell、Redis 未授权写 crontab、C2 心跳、SSRF 云元数据、勒索落地、PsExec 横移 + 2 题良性对照：正常备份任务/K8s 探活，防"见规律就报警"），10 题关卡的采样噪声减半；③ D3 全部 `incident` 题理由关键词组剔除日志原文词（root/ssh/failed/cmd/事件 ID 等），只留归纳性措辞，堵死"复读日志蹭理由分"；runner 记录 `finish_reason`（OpenAI 系 stop/length、Anthropic 系 end_turn/max_tokens），截断作答单独标注并入结果 JSON 的 `truncated` 字段，思考链（reasoning_content）只存档不参与判分，杜绝长思考耗尽 token 时把原始推理当答案蹭关键词。
